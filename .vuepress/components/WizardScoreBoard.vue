...
.container(v-cloak)
  ...
  .card.mb-3
    ...
    .card-body.p-2.border-0(v-if="curTab == 'rule'")
      ...
      p {{ players.length }} 人遊戲共有 {{ _.inRange(players.length, 3, 7) ? (60 / players.length) : 0 }} 局。 每局由一個人順時針輪流作為發牌玩家，第 1 局每人 1 張牌、第 2 局每人 2 張牌…依此類推，發牌玩家需要先把牌發給自己。
      p 沒發完的牌要翻出最上面的一張作為王牌，王牌的花色即為該局的「王牌色」。 若王牌為 Z 則由發牌玩家決定本局的王牌色，若王牌為 N 或是最後一局時，則以「無王牌色」進行。
      p 每局開始時，由發牌玩家開始預測本局會吃幾墩，其他人順時針依序喊出預測會吃幾墩，需要用本記分板紀錄所有人所預測的墩數。
      p 預測完後就進入出牌階段，每次由本局吃到上一墩的玩家先發，若是本局第一次出牌則由發牌者先發。
      p 其他玩家必須順時針依序打出一張與先發者同花色的牌（此時也可打出 Z 和 N），如果沒有同花色的牌，才能夠打出王牌色或其他花色的牌。
      p 若先發者打出無花色的牌（Z 或 N），則由下一家出牌決定本次的花色。 若下一家又打出無花色的牌，則再由下一家出牌決定。
      p
        | 所有玩家都出一張牌以後，牌最大的玩家吃到這一墩，並且把這一墩的牌收到自己面前，牌的大小為：
        ol
      ...
      p 當本局的手牌全部打完後，本局結束，並且將本局每個人所吃到的墩數紀錄於本記分板。
      p
        | 遊戲計分的方式為：
        ul
          li 若該局玩家預測正確，則分數為：墩數乘以 10 再加上額外的 20 分。
          li 若該局玩家預測錯誤，則分數為：多吃或少吃的墩數乘以 -10。（不能額外加 20 分）
      p.mb-0 玩家每局所獲得的分數加總即為總分，總分最多的人獲勝。
